...
class DatabaseHealthChecker:
    ...
    def _get_raw_connection(self):
        if hasattr(self.connector, "_pool") and self.connector._pool is not None:
            return self.connector._pool.acquire()
        if hasattr(self.connector, "_conn") and self.connector._conn is not None:
            return self.connector._conn
        return None

    def _release_raw_connection(self, conn):
        if hasattr(self.connector, "_pool") and self.connector._pool is not None:
            self.connector._pool.release(conn)
    ...
    async def _get_mysql_table_stats(self) -> list[TableHealth]:
        ...
        try:
            ...
            self._release_raw_connection(conn)
    ...
    async def _get_mongodb_stats(self) -> list[TableHealth]:
        ...
        try:
            def _run():
                db = conn[self.connector.database]
